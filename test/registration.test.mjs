// Tool-surface invariants for @three-ws/agenc-mcp.
//
...
//
// Run: node --test packages/agenc-mcp/test/registration.test.mjs
...
test('every tool is a read-only, live-data query', () => {
	for (const tool of TOOLS) {
		// This server is the FREE read-first AgenC surface. link_agent derives an
		// identity and reads chain state — it registers/signs nothing — so every
		// tool here is honestly read-only.
		...
		// Live on-chain/marketplace data is never idempotent — state moves between calls.
